...
# detect config/menu dir (prefer panels/menu)
dir_candidates=(
...
# Launch Rofi with theme file found under DIR/<theme>/styles.rasi
if [ ! -f "$DIR/$theme/styles.rasi" ]; then
    rofi -e "Theme not found: $DIR/$theme/styles.rasi"
    exit 1
fi

MENU="$(rofi -no-config -no-lazy-grab -sep "|" -dmenu -i -p '' \
-theme "$DIR/$theme/styles.rasi" \
<<< " Budgie| Deepin| Elementary| Elementary_Dark| Gnome| KDE| KDE_Dark| Liri| Mint| Ubuntu_gnome| Ubuntu_unity| Xubuntu| Zorin|")"
